...
class TestDeleteNoteViews(object):
    ...
    def test_delete_note(self, client, note, unfresh_token, session):
        ...
        assert response_data['message'] == 'Note {0} deleted'.format(note.id)

    def test_delete_from_refresh(self, client, note, refresh_token, session):
        url = '{0}/{1}'.format(URL, note.id)
